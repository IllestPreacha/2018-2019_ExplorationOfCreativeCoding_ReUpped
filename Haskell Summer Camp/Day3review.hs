...
-- a doesnt matter in the above statement since b is the one use
-- since it is a m "a" and not just an "a", it stil has a reasonabe component to 
...
main = do
  ...
  X <- getLine
  putStrLn $ "you wrote: " ++ x :: IO()

main = putStrLn " hi there" >> (getLine >>= (\x -> (putStrLn $ "you wrote: " ++ x))
--    		               m a           m b            =   becomes m b


repeater :: IO ()
repeater = getLine >>= putStrLn
